...
export const init = () => {
    ...
    // Handle the removal of partnership headers
    $("body").on("click", ".remove-partnership", function () {
...
define(["jquery", "core/str", "core/notification"], ($, Str, Notification) => {
     ...
     */
    const init = () => {
        const form = $("form#mform1");

        form.on("click", ".remove-partnership", (e) => {
            e.preventDefault();
            const partnership = $(e.currentTarget).closest("fieldset");
            const partnerships = form.find("fieldset");

            if (partnerships.length > 1 && !partnership.is(":first-of-type")) {
                partnership.remove();
            } else if (partnership.is(":first-of-type")) {
                Str.get_string(
                    "cannotremovefirstpartnership",
                    "local_equipment"
                )
                    .then((string) => Notification.alert("", string))
