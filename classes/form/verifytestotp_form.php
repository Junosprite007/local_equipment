...
class verifytestotp_form extends \moodleform {
    public function definition() {
        $mform = $this->_form; // Don't forget the underscore!

        $mform->addElement('text', 'otp', get_string('otp', 'local_equipment')); // Add elements
        $mform->setType('otp', PARAM_NOTAGS); //Set type of element
        $mform->addRule('otp', null, 'required', null, 'client');

        $this->add_action_buttons(false, get_string('verifyotp', 'local_equipment'));
    }
    ...
    public function validation($data, $files): array {
        ...
        if (isset($data['otp'])) {
            ...
            $messages = array();
            if (!is_number($otp)) {
                array_push($messages, get_string('enternumbersonly', 'local_equipment'));
            }
            if (strlen($otp) != 6) {
                array_push($messages, get_string('enterexactly6digits', 'local_equipment'));
            }
            if (count($messages) > 0) {
